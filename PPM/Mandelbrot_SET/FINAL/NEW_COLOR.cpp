...
int index_map[MAX_H+20][MAX_W+2];
double gam=5.0000f;
...
inline void slow_YIQ2RGB( double target[3], double mtx[3] ){
	...
	for(int i=0; i<3; ++i) if(target[i]>0.99999f) target[i]=0.999997f;
}
...
int main(void)
{
	...
	for(int k=0; k<term; k++)
	{
		...
		for(int i=0; i<h; ++i)for(int j=0; j<w; ++j) ++hist[ (unsigned)((double)index_map[i][j] / Q) ];
		det_luminance();
		for(int i=0;i<img.getX();i++)
		{
			for( int j=img.getY()-1;j>=0;--j)
			{
				...
				if(!index_map[i][j]){si.R=si.G=si.B=0;}
					...
					si.R = (unsigned char)floor((COLMTX[0] * 255.0f)-0.5f);
					si.G = (unsigned char)floor((COLMTX[1] * 255.0f)-0.5f);
					si.B = (unsigned char)floor((COLMTX[2] * 255.0f)-0.5f);
				}
				img.setCOL(i,j,si);
			}
		}
