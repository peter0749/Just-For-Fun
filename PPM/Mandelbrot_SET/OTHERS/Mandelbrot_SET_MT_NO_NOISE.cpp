...
inline bool DRAWPOINT(const int r, const int x, const int y, const int tarx, const int tary, Draw &IMG, const COLOR p)
{
    ...
    ty = tary-y;
    dist = tx*tx+ty*ty;
    if(dist<tr && tarx>=0 && tary>=0 && tarx<IMG.getX() && tary<IMG.getY())
    {
        IMG.setCOL(tarx,tary,p);
        return true;
    }
    return false;
}
...
{
    int h, w, d;
    int c_x, c_y, tx, ty, kx, ky, den, cr, t;
    int step, total, old, est;
