...
/*Choose better pivot*/
	...
	memcpy(ptr+rd_m*block, ptr+right*block, word_len);
	memcpy(ptr+right*block, t2, word_len);
    /*begin to partition.*/
	...
	while( i<j )
	{
		if( (pa=cmp(ptr+i*block, ptr+right*block)) < 0 )
		{
			memcpy(t2, ptr+k*block, word_len);
			memcpy(ptr+k*block, ptr+i*block, word_len);
			memcpy(ptr+i*block, t2, word_len);
			i++;k++;
		}
		else if( pa==0 )
		{
			j--;
			memcpy(t2, ptr+i*block, word_len);
			memcpy(ptr+i*block, ptr+j*block, word_len);
			memcpy(ptr+j*block, t2, word_len);
		}
		else i++;

	}
	rd_m = j-k < right-j+left ? j-k : right-j+left;
	pa = j;
	for(i=k, j=right-rd_m+1; i<k+rd_m && j<=right; i++,j++)
	{
		memcpy(t2,ptr+i*block,word_len);
		memcpy(ptr+i*block, ptr+j*block, word_len);
		memcpy(ptr+j*block, t2, word_len);
    ...
    //free(temp);
    /*End of partition.*/
    real_sort(left,k-1,ptr,block,word_len,cmp);
	real_sort(right-pa+k+1, right,ptr,block,word_len,cmp);
}
