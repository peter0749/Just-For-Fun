...
void prtout(int crr_level)
{
	...
	for(i=0; i<MAXN; i++)
	{
		for(j=0; j<MAXN; j++)
		{
			fprintf(stdout,maze[crr_level][i][j] ? (maze[crr_level][i][j]==2?" o":" #"):"  ");
		}
		fprintf(stdout,"\n");
	}
	//printf("狀態\nO:\t抵達\nW:\t撞牆\nM:\t已拜訪過\nX:\t死路");
...
void prtchar(int crr_row, int crr_col, int crr_dir, int crr_level)
{
	...
	else
	{
		...
		fprintf(stdout,maze[pr_level][pr_row][pr_col] ? (maze[pr_level][pr_row][pr_col]==2?" o":" #"):"  ");
		fflush(stdout);
	}
